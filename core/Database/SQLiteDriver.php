    ...
    private function handleSelect(QueryBuilder $builder): array
    {
        ...
        if (!empty($builder->unselect)) {
            if (count($select) === 1 && $select[0] === '*') {
                $stmt = $this->connection->query("SELECT name FROM pragma_table_info('{$builder->table}')");
    ...
    protected function buildColumnDefinition(array $col): string
    {
        ...
        switch ($type) {
            ...
            case 'STRING':
                ...
                break;
            case 'INTEGER':
                $sqlType = ($col['unsigned'] ?? false) ? "INTEGER" : "INTEGER";
                break;
            case 'TINYINT':
                $sqlType = ($col['unsigned'] ?? false) ? "TINYINTEGER" : "INTEGER";
                break;
            case 'BIG_INTEGER':
                $sqlType = ($col['unsigned'] ?? false) ? "BIGINTEGER" : "INTEGER";
                break;
            case 'BOOLEAN':
                $sqlType = "TINYINT(1)";
                break;
